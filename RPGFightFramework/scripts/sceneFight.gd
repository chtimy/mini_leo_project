...
func init(var mapFilePath, var turnScriptPath, var selectables):
	# action generation
	m_actionsDico = load("res://RPGFightFramework/scripts/actions.gd").new()
	...
	if script == null:
		...
		for selectable in selectables:
			if selectable.isCategory("Players"):
				...
				j += 1
				selectable.initMenu(get_node("."), m_actionsDico)
